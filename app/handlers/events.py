        ...
        for admin in admins:
            ...
            if admin.status == "creator":
                ...
                statement = select(GroupAdmin).where(
                    ...
                    GroupAdmin.user_id == owner_user.id
                )
                ...
                if not existing_admin:
                    ...
                    owner_admin = GroupAdmin(
                        ...
                        full_name=owner_user.full_name or owner_user.first_name or "Owner",
...
async def on_chat_member_updated(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with Session(engine) as session:
        ...
        if new_status in member_statuses:
            ...
            inviter_id = chat_member_update.from_user.id if chat_member_update.from_user else None
...
async def check_unbound_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with Session(engine) as session:
        ...
        # 检查是否开启了自动删除未绑定频道消息
        auto_delete_unbound_channel = group.config.get('auto_delete_unbound_channel', False)
...
async def _delete_channel_message(update: Update, context: ContextTypes.DEFAULT_TYPE, sender_chat):
    ...
    try:
        ...
        # 清理过期的限流记录（超过120秒的）
        expired_keys = [
            key for key, last_time in channel_tip_rate_limit.items()
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (update.effective_user and
        ...
        update.message.text in ['确认', '确定', 'confirm']):
    ...
    with Session(engine) as session:
        ...
        if is_channel:
            ...
            statement = select(GroupMember).where(
                GroupMember.group_id == group.id,
                GroupMember.user_id == sender_chat_id
            )
            member = session.exec(statement).first()
            ...
            if not member:
                ...
                if update.effective_user.username and member.username != update.effective_user.username:
...
async def _batch_kick_users(bot, group_id: int, users_to_kick: list, status_message, status_chat_id: int):
    """后台异步批量踢出用户"""
    kicked_count = 0
    failed_count = 0
...
async def _handle_confirm_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'pending_kick' in context.bot_data and reply_to_msg_id in context.bot_data['pending_kick']:
        # 获取待踢出的信息
...
async def _detect_and_react_to_image(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if user_id and user_id in image_queue.image_blacklist:
        ...
        if time.time() < blacklist_until:
            ...
            logger.debug(f"Skipping image from blacklisted user {user_id} (message {message.message_id}, {remaining}s remaining)")
            return
    ...
    try:
        ...
        # 将任务加入队列（非阻塞）
        enqueued = await image_queue.enqueue(update, context, message, bytes(image_bytes))
        if enqueued:
            logger.debug(f"Image detection task enqueued for message {message.message_id}")
        else:
            logger.debug(f"Image task skipped for message {message.message_id} (duplicate/rate-limited)")

    except Exception as e:
        logger.error(f"Error enqueuing image detection for message {message.message_id}: {e}")
