...
async def dm_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 解析目标用户ID和消息
    try:
        target_user_id = int(context.args[0].replace('@', ''))
    except ValueError:
    ...
    with Session(engine) as session:
        ...
        try:
            # 创建已读回执按钮
            keyboard = [[
                InlineKeyboardButton("✅ 标记为已读", callback_data=f"dm_read_{dm_relay.id}")
            ]]
...
async def dm_read_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with Session(engine) as session:
        ...
        receipt = DMReadReceipt(
            dm_relay_id=dm_relay.id,
            read_by=update.effective_user.id
        )
        session.add(receipt)
        session.commit()
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    f"✅ 你发送给用户 {dm_relay.to_user_id} 的私信已被阅读\n"
...
async def my_dms_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with Session(engine) as session:
        ...
        if sent_dms:
            text += "<b>📤 已发送:</b>\n"
            for dm in sent_dms:
                status = "✅已读" if dm.read else ("📨已送达" if dm.delivered else "❌未送达")
                text += (
                    f"→ 用户 {dm.to_user_id}: "
                    f"{dm.message[:30]}... [{status}]\n"
                )
            text += "\n"
...
dm_handlers = [
    CallbackQueryHandler(dm_read_callback, pattern="^dm_read_")
]
